...
public class ExternalProbe extends Subsystem.Deployment {
  private Host host;
  
  public final CommandDescriptor[] directory = new CommandDescriptor[1];
  
  public final CommandDescriptor[] execution = new CommandDescriptor[1];
  
  public ExternalProbe() {
    
  }
  
  public ExternalProbe(final Host host) {
    this.host = host;
  ...
  public void setup() {
    ...
    execution[0].runsOn(hosts0);
    execution[0].dependsOn(directory[0]);
  }
  ...
  private CommandDescriptor initExecution0() {
    return Commands.run()
        .withComposite("external-probe")
