...
public class R_1CO_12NO_5MB {
  ...
  private void custom() {
    try {
      ...
      final int numComponents = 1;
      final int jMeterMB = 5;
    ...
    if (＿subsystems.containsKey(clazz)) {
      ＿subsystems.get(clazz).add(new Subsystem(clazz, deployment));
    	...
    	throw new RuntimeException("Subsystem '" + clazz + "' has not been included in " + 
    		"deployment 'edu.icesi.amelia.validation.cts.deployments.R_1CO_12NO_5MB'");
    }
  }
