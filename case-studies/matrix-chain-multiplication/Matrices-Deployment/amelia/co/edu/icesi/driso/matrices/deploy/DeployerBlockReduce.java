...
public class DeployerBlockReduce {
  ...
  private void custom() {
    try {
      final Integer strategy = Integer.valueOf(1);
          ...
          public Boolean apply() {
            try {
              boolean _xblockexpression = false;
              {
                ...
                DeployerBlockReduce.this.add(_common);
                Compile _compile = new Compile(strategy, hosts);
                DeployerBlockReduce.this.add(_compile);
                Allocation _allocation = new Allocation(executionHosts, hosts);
                DeployerBlockReduce.this.add(_allocation);
                BlockReduce _blockReduce = new BlockReduce(executionHosts, hosts);
                DeployerBlockReduce.this.add(_blockReduce);
                _xblockexpression = DeployerBlockReduce.this.start(true);
              }
    ...
    if (＿subsystems.containsKey(clazz)) {
      ＿subsystems.get(clazz).add(new Subsystem(clazz, deployment));
    	...
    	throw new RuntimeException("Subsystem '" + clazz + "' has not been included in " + 
    		"deployment 'co.edu.icesi.driso.matrices.deploy.DeployerBlockReduce'");
    }
  }
